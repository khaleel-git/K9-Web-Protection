...
function toggleDefaultPw(el) {
  const dots = el.querySelector('.pw-dots')
  const text = el.querySelector('.pw-text')
  const hint = el.querySelector('.pw-hint')
  const showing = text.style.display !== 'none'
  dots.style.display = showing ? '' : 'none'
  text.style.display = showing ? 'none' : ''
  hint.textContent = showing ? 'click to reveal' : 'click to hide'
}
window.toggleDefaultPw = toggleDefaultPw
